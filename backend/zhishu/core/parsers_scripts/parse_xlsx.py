"""智枢解析插件脚本 —— Excel(.xlsx/.xls) 文本提取（委托标准库，零依赖）。

    python parse_xlsx.py <文件路径>

统一复用 zhishu.core.rag.read_file_text 的零依赖标准库提取逻辑，
不再依赖 openpyxl，保证「安装即可用、调用必返回内容」。
无论模型调用 read_file 还是 parse_xlsx，都能拿到一致的工作表文本。
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        sys.stderr.write("用法：python parse_xlsx.py <文件路径>\n")
        return 2
    path = sys.argv[1]
    try:
    ...
    try:
        ...
        sys.stderr.write(f"Excel 解析失败：{e}\n")
        return 2

    if not text.strip():
        sys.stderr.write("Excel 文档未提取到内容（旧版 .xls 暂不支持，请另存为 .xlsx）。\n")
        return 3
