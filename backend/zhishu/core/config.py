...
class AgentConfig:
    ...
    # 迭代预算上限（对齐 Hermes IterationBudget：父 Agent 默认 90 / 子 Agent 50）：
    # 单次 run 最多执行的 LLM 推理轮数（一轮可含多条并行工具调用）。这是「保证任务完成」
    # 的主安全阀——预算内 Agent 自由推进，不再因「相同参数重复 N 次」就被硬掐断；预算
    # 耗尽前注入「收尾」提醒（grace call），确保 Agent 能给出最终结论而非半路被杀。
    # 子 Agent（被委派）可经 agent meta 的 max_steps 单独下调（如 50）以控递归成本。
    max_steps: int = 90
    # 同一 LLM 响应返回多个「非委派」叶子工具时并发执行（I/O 密集，安全），
    # 缩短多工具步的端到端耗时（对标 Hermes 分段并行）。关闭则回退串行。
    parallel_tools: bool = True
    parallel_tool_workers: int = 5            # 叶子工具并发上限（信号量）
    # 连续失败软提醒阈值（不再硬终止）：连续 N 次工具失败/被拦截即注入「停止无意义重试」
    # 系统提醒，引导 Agent 改用其他命令/工具或直接说明障碍；任务是否继续由 max_steps 预算兜底。
    tool_fail_break: int = 6
    # 重复失败循环软提醒阈值（不再硬终止）：同一 (工具名, 归一化参数) 反复失败累计 N 次即
    # 注入提醒，兜底捕获「code_exec 装 Node → terminal_run 装 Node 被拦 → 再 code_exec…」式
    # 死循环（成功/失败交替使连续失败计数被清零，故需独立按签名累计）。仅提醒，不掐断。
    tool_cycle_break: int = 4
    # 工具步骤软上限（不再硬终止）：超过后注入「请收尾」提醒而非掐断任务，
    # 真正的硬终止仅由 max_steps 迭代预算兜底。默认给足余量，正常任务不会触达；
    # 设小可用于在并行工具密集场景下更早提示 Agent 收敛。
    max_tool_steps: int = 300
    # 重复成功循环软提醒阈值（Task #399，不再硬终止）：同一 (工具名, 归一化参数) 被反复调用
    # 且**均成功返回**累计 N 次，转为「跳过冗余执行 + 停止重复提醒」，打破「反复 read_file
    # 同一文档 / 反复调同一只读工具」式停滞。仅跳过冗余、不掐断任务，运行继续由 max_steps 兜底；
