"""检索问句改写（对标 Hermes `plugins/memory/query_rewrite.py`）。

Provider 无关：任何记忆 provider 都可把 ``rewrite_memory_query`` 当作检索引子的改写器。
模型/超时由调用方（MemoryManager）注入的 ``llm_call`` 决定。

行为：把用户最新一句话改写为一个关于「该用户的历史/偏好/过往决策」的检索问句，
交给记忆系统做语义召回。模型调用失败或结果不安全时回退为 ""（保持旧行为）。
"""
...
def _extract_text(resp) -> str:
    """兼容两种返回形态：① 直接字符串；② OpenAI 风格响应对象（.choices[0].message.content）。"""
...
def rewrite_memory_query(user_message: str, llm_call=None) -> str:
    """Return a retrieval-only question, or ``""`` to preserve old behavior.

    ``llm_call`` 为同步可调用 ``(messages) -> str|response``；不传则直接返回 ""。
    """
    if llm_call is None:
        return ""
    ...
    try:
        ...
        rewritten = _normalize_rewrite(_extract_text(resp))
        if not rewritten:
