...
def _needs_supervisor_delegation(text: str) -> bool:
    ...
    极短问句等简单对话应允许直接作答，否则「你好」也会被派给投资总监。
    判定口径刻意保守：只排除明确的闲聊与过短输入，其余一律视为需要委派。
    """
    t = (text or "").strip()
    if len(t) < 6:
        return False
...
class Agent:
    ...
    async def run(
        ...
        if agent_name:
            ...
            if not can_delegate:
                specs = [s for s in specs if s["function"]["name"] != DELEGATE_TOOL_NAME]
            # 主管模式下若存在协调类子智能体，强制走委派链路：裁剪 web/执行类工具，
            # 仅保留 delegate_to_agent 与少量元工具（历史回忆、记忆、任务清单、知识库/读文件）。
            # 这是防止主管把复合任务（如股票分析）当成普通问答自己执行 16 步的关键闸门。
            if can_delegate and has_coordinator:
                allowed = {
