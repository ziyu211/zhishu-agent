...
**你必须在单个回合里尽量一次性发出多个相互独立的工具调用**——系统会并发执行它们，把 N 次串行往返压缩成 1 次。请严格按以下优先级：

1) 同类重复调用 → 用批量参数，1 次调用搞定（首选，结果最紧凑）：
...
2) 异类 / 多工具独立调用 → 在同一回合里**并列发出多个 tool_call**（系统并发执行，不是一个个等）：
   例如「读 3 个文件 + 跑 2 条命令 + 执行 1 段代码」应当是一回合里 6 个并列的 tool_call，而不是 6 个回合各 1 个。

- 仅当后续调用**强依赖**上一步的返回值时，才分回合（例如必须先 read_file 拿到文件名，才能 code_exec 处理它）。
- 反例（绝对不要）：`read_file(a)` → 等结果 → `read_file(b)` → 等结果 … 反复 14 次往返。
- 正例（应该这样）：`read_file(paths=[a,b,...,n])` 一次往返；或在**同一回合**并列发出全部 14 个 `read_file`（系统并发执行，仍是 1 次往返）。
