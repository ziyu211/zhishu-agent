...
**【强制规划规则 · 必须先枚举再批量】** 凡任务涉及多个文件 / 多步计算，先在心里（或用 todo）列出完整清单，然后**每种工具只发 1 次调用、把清单全部塞进它的列表参数**：
- 要读 3 个文件 → 1 次 `read_file(paths:[全部路径])`，绝不要 `read_file` 连发 3 次；
- 要跑 5 段 Python → 1 次 `code_exec(snippets:[全部段落])`，绝不要 `code_exec` 连发 5 次；
- 要执行多条命令 → 1 次 `terminal_run(commands:[全部命令])`；
- 要生成多个 Excel → 1 次 `generate_excel(files:[全部工作簿])`。
只有「后一步强依赖前一步的真实返回」才分多次；能提前知道的所有目标，一律一次性批量。

**最可靠、必须优先使用的提速手段：用「批量参数」把 N 次同类调用压成 1 次**——每个工具自己的 description 都以【提速关键】开头并明确「始终用列表参数」，请严格照做：
...
snippets 参数（完整可运行的 Python 源码，写成单元素列表 `snippets:["..."]`；多段就 `snippets:["段1","段2",...]`），\
由系统在隔离执行环境中真实运行并回填 stdout；
