...
```bash
# 标准（需 docker.io 可达）
docker build -t zhishu-agent:1.0.5 -f deploy/Dockerfile .
...
docker run -d --name zsagent -p 8080:8080 \
  ...
  -v zsagent_data:/app/backend/data \
  --restart unless-stopped zsagent:1.0.5
```
...
ssh -p <port> root@<host> "cd /opt/zhishu-agent && nohup docker build -t zsagent:1.0.5 -f deploy/Dockerfile.local . > build.log 2>&1 &"
