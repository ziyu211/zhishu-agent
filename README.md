...
```bash
# 标准（需 docker.io 可达）
docker build -t zhishu-agent:1.0.12 -f deploy/Dockerfile .

# 受限网络 / 国内零代理（推荐内网、本机）
docker build -t zsagent:1.0.12 -f deploy/Dockerfile.local .
```
...
docker run -d --name zsagent -p 8080:8080 \
  ...
  -v zsagent_data:/app/backend/data \
  --restart unless-stopped zsagent:1.0.12
```
...
ssh -p <port> root@<host> "cd /opt/zhishu-agent && nohup docker build -t zsagent:1.0.12 -f deploy/Dockerfile.local . > build.log 2>&1 &"
...
docker build --platform linux/arm64 -t zsagent:1.0.12-arm64 -f deploy/Dockerfile.local .
```
